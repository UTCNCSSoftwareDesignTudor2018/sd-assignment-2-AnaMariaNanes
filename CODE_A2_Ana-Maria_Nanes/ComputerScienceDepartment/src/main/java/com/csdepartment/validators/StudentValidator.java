...
public class StudentValidator {
	...
	public String validateStudent(Student st)
	{
        ...
        if(st.getCard() == null || st.getCard().equals("")  || st.getCard().length() != 5 || ( ! st.getCard().matches("[0-9]+")) || (! isCardIDUnique(st.getCard())))
        	return message = "Wrong cardID field format";
        if(st.getCnp()== null || st.getCnp().equals("")  || st.getCnp().length() != 10|| ( ! st.getCard().matches("[0-9]+")) || (! isCnpUnique(st.getCnp()))) 
        	return message = "Wrong cnp field format";
        if(st.getAddress() == null || st.getAddress().equals("")  ) 
        	return message = "Wrong address field format";
        if(st.getGroupid() == null || st.getGroupid().equals("")  || st.getGroupid().length() != 5 || ( ! st.getCard().matches("[0-9]+"))) 
        ...
        if(st.getPassword() == null || st.getPassword().equals("")  || st.getPassword().length() != 5 || ( ! st.getCard().matches("[0-9]+"))) 
	...
	public String validateUpdateStudent(Student st)
	{
        ...
        if(st.getName().equals("") || st.getName().equals("") ) 
        	return message = "Wrong name field format";
        if(st.getCard() == null || st.getCard().equals("")  || st.getCard().length() != 5 || ( ! st.getCard().matches("[0-9]+")) )
        	return message = "Wrong cardID field format";
        if(st.getCnp()== null  || st.getCnp().equals("") || st.getCnp().length() != 10|| ( ! st.getCard().matches("[0-9]+")) ) 
        	return message = "Wrong cnp field format";
        if(st.getAddress() == null || st.getAddress().equals("") ) 
        	return message = "Wrong address field format";
        if(st.getGroupid() == null || st.getGroupid().equals("")  || st.getGroupid().length() != 5 || ( ! st.getCard().matches("[0-9]+"))) 
